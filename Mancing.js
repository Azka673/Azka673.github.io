...
let state = { 
    ...
    rod: 'wood', 
    owned: ['wood'],
    baseSpeed: 4.5 // Kecepatan normal bar putih
};

// LOAD DATA
if(localStorage.getItem('ocean_rng_save_v5')) {
    const saved = JSON.parse(localStorage.getItem('ocean_rng_save_v5'));
    state = { ...state, ...saved };
}
...
let dir = 1; 
let moving = true; 
let isSlowing = false; // Flag untuk mendeteksi mode lambat setelah strike
...
// CORE LOOP - KECEPATAN DIPERBAIKI
function loop() {
    let currentSpeed;
    
    if (moving) {
        currentSpeed = state.baseSpeed; // Kecepatan normal
    } else if (isSlowing) {
        currentSpeed = 0.8; // Kecepatan SANGAT LAMBAT setelah strike
    } else {
        currentSpeed = 0; // Berhenti total jika diperlukan
    }

    pos += currentSpeed * dir;
...
// CATCH LOGIC - PERLAMBATAN REEL
document.getElementById('catch-btn').onclick = () => {
    if(!moving) return;
    
    moving = false; 
    isSlowing = true; // Mengaktifkan mode lambat di fungsi loop
    ...
    // Tunggu sebentar sebelum bar kembali bergerak normal
    setTimeout(() => { 
        isSlowing = false;
        moving = true; 
    }, 1000); 
...
function buyRod(k) { 
    if(state.money >= rods[k].cost) { 
        state.money -= rods[k].cost; 
        state.owned.push(k); 
        save(); 
        renderShop(); 
    } 
...
function sellAllFish() { 
    let e = 0; 
    state.inventory.forEach(f => e += f.price); 
    state.money += e; 
    state.inventory = []; 
    save(); 
    renderInv(); 
...
function buyUpgrade(t) {
    if(t === 'luck' && state.money >= state.luckLv * 250) { 
        state.money -= state.luckLv * 250; 
        state.luckLv++; 
    }
    if(t === 'hook' && state.hookLv < 10 && state.money >= (state.hookLv + 1) * 1200) { 
        state.money -= (state.hookLv + 1) * 1200; 
        state.hookLv++; 
    }
    save();
}
